...
export class Calendar extends Panel {
    ...
    protected addChildren(): void {
        ...
        for(let i: number = 0; i < 6; i++) {
            for(let j: number = 0; j < 7; j++) {
                let btn: PushButton = new PushButton(this.content, j * 20, 20 + i * 20);
                btn.setSize(19, 19);
        ...
        let prevYearBtn: PushButton = new PushButton(this.content, 2, 2, "«", this.onPrevYear);
        prevYearBtn.setSize(14, 14);
        let prevMonthBtn: PushButton = new PushButton(this.content, 17, 2, "<", this.onPrevMonth);
        prevMonthBtn.setSize(14, 14);
        let nextMonthBtn: PushButton = new PushButton(this.content, 108, 2, ">", this.onNextMonth);
        nextMonthBtn.setSize(14, 14);
        let nextYearBtn: PushButton = new PushButton(this.content, 124, 2, "»", this.onNextYear);
    ...
    public setDate(date: Date): void {
        ...
        this._day = date.date;
        let startDay: number = new Date(this._year, this._month, 1).day;
        let endDay: number = this.getEndDay(this._month, this._year);
        for(let i: number = 0; i < 42; i++) {
            this._dayButtons[i].visible = false;
        }
        for(i = 0; i < endDay; i++) {
            let btn: PushButton = this._dayButtons[i + startDay];
            btn.visible = true;
