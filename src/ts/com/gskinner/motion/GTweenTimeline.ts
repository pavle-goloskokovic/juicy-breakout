...
export class GTweenTimeline extends GTween {
    ...
    public set position(value: number) {
        let tmpSuppressEvents: boolean = this.suppressEvents;
        this.suppressEvents = true;
        super.position = value;
        let repeatIndex: number = this._position / this.duration >> 0;
        let rev: boolean = this.reflect && repeatIndex % 2 >= 1;
        let i: number;
        let l: number = this.tweens.length;
        if(rev ) {
            for(i = 0; i < l; i++) {
    ...
    public addCallback(labelOrPosition: any, forwardCallback: Function, forwardParameters: any[] = null, reverseCallback: Function = null, reverseParameters: any[] = null): void {
        let position: number = this.resolveLabelOrPosition(labelOrPosition);
        if(isNaN(position) ) {
            return;
        } 
        let callback: Callback = new Callback(position, forwardCallback, forwardParameters, reverseCallback, reverseParameters);
        let l: number = this.callbacks.length;
    ...
    public goto(labelOrPosition: any): void {
        let pos: number = this.resolveLabelOrPosition(labelOrPosition);
        if(!isNaN(pos) ) {
            this.position = pos;
        } 
    }
    ...
    protected checkCallbacks(): void {
        ...
        let repeatIndex: number = this._position / this.duration >> 0;
        let previousRepeatIndex: number = this.positionOld / this.duration >> 0;
    ...
    protected checkCallbackRange(startPos: number, endPos: number, includeStart: boolean = false): void {
        ...
        while((i += k) != j) {
            let callback: Callback = this.callbacks[i];
            let pos: number = callback.position;
