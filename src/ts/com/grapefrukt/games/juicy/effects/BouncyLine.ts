...
export class BouncyLine extends GameObject {
    ...
    public wobble(x: number, y: number): void {
        let wobble_pos: Point = new Point(x - this.pos_middle.x, y - this.pos_middle.y);
        let tx: number = wobble_pos.x;
    ...
    public checkCollision(ball: Ball): void {
        ...
        let dist: number = this.distanceFromLine(this.pos1, this.pos2, new Point(ball.x, ball.y));
        let max_distance: number = 0.5 * Settings.EFFECT_BOUNCY_LINES_WIDTH + Settings.EFFECT_BOUNCY_LINES_DISTANCE_FROM_WALLS;
    ...
    public closestPointOnLineSegment(a: Point, b: Point, p: Point): Point {
        let c: Point = new Point(p.x - a.x, p.y - a.y);
        let v: Point = new Point(b.x - a.x, b.y - a.y);
        let distance: number = v.length;
        if(distance != 0 ) {
            v.x /= distance;
            v.y /= distance;
        } 
        let t: number = v.x * c.x + v.y * c.y;
        if(t < 0 ) {
            return a.clone()
        } 
        if(t > distance ) {
    ...
    public distanceFromLine(a: Point, b: Point, p: Point): number {
        let delta: Point = this.closestPointOnLineSegment(a, b, p);
        delta.x = delta.x - p.x;
        delta.y = delta.y - p.y;
        return delta.length;
    }
