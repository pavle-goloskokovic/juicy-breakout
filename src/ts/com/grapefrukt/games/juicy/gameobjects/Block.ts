...
export class Block extends GameObject {
    ...
    public constructor(x: number, y: number) {
        ...
        if(Settings.EFFECT_TWEENIN_ENABLED ) {
            ...
            let t: GTween = new GTween(this._gfx, Settings.EFFECT_TWEENIN_DURATION);
            t.proxy.y = 0;
            t.proxy.rotation = 0;
            t.proxy.scaleX = 1;
            t.proxy.scaleY = 1;
            t.delay = Math.random() * Settings.EFFECT_TWEENIN_DELAY;
            let easing: Array<Function> = Array([Linear.easeNone, Quadratic.easeOut, Back.easeOut, Bounce.easeOut]);
    ...
    public collide(ball: Ball): void {
        ...
        if(Settings.EFFECT_BLOCK_SCALE ) {
            for(let slice of this._sliceEffect.slices) {
