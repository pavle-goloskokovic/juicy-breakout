...
export class Toggler extends Sprite {
    ...
    private reset (): void
    {
        const typeXML: XML = describeType(this._targetClass);
        this._properties = [];
        let property: Property;
        let tag: XML;
        for (const variable of typeXML.variable)
        {
        ...
        const settingWindow: Window = new Window(this, 10, 10);
        settingWindow.title = 'JUICEATRON 5002 XX';
        settingWindow.width = 250;
        settingWindow.height = Settings.STAGE_H - 50;
        const accordion: Accordion = new Accordion(settingWindow);
        let window: Window;
        for (property of this._properties)
        {
            if (!window || window.title != property.header && property.header != '' )
            {
                ...
                const container: VBox = new VBox(window.content, 10, 10);
            }
            const row: HBox = new HBox(DisplayObjectContainer(window.content.getChildAt(0)));
            const label: Label = new Label(row, 0, 0, this.prettify(property.name));
            ...
            switch (property.type)
            {
                case 'Boolean':
                    {
                        const checkbox: CheckBox = new CheckBox(row, 0, 0, '', this.getToggleClosure(property.name));
                ...
                case 'int':
                    {
                        const slider: HUISlider = new HUISlider(row, 0, 0, '', this.getSliderClosure(property.name));
                        slider.width = 130;
