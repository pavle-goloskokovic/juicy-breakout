...
export class SoundControl extends EventDispatcher {
    ...
    public play(id: string): void {
        ...
        let group: any[] = this._groups[id];
        if(group ) {
            id = group[int(Math.random() * group.length)].id
        } 
        let sound: SoundObject = this.getSound(id);
        sound.play();
    }
    ...
    private parseXML(): void {
        ...
        for(let group of this._xml_config.sound) {
            this.registerGroup(group)
        }
    }
    private registerSound(id: string, file: string, data: XML): SoundObject {
        let volume: number = data.volume || 1;
        let pan: number = data.pan || 0;
        let startTime: number = data.startTime || 0;
        let loops: number = data.loops || 0;
        let soundObject: SoundObject = new SoundObject(id, file, volume, pan, startTime, loops, this._embed_class);
    ...
    private registerGroup(group: XML): void {
        let i: number = 0;
        for(let file of group.file) {
            let soundObject: SoundObject = this.registerSound(group.id + "-" + i.toString(), file, group);
